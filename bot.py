...
async def start(m: Message, state: FSMContext):
    ...
    if m.from_user.id not in allowed:
        await m.answer(
            "🔒 <b>Доступ ещё не открыт.</b>\n\n"
            "Чтобы пользоваться ботом, нужно один раз получить доступ. Сделайте так:\n\n"
            "1. Отправьте мне команду <b>/me</b> — я покажу ваш Telegram ID.\n"
            "2. Скопируйте этот номер и отправьте его администратору бота "
            "(тому, кто вам дал ссылку на бота).\n"
            "3. Администратор добавит вас командой /add <ваш_ID>.\n"
            "4. После этого нажмите /start ещё раз — и бот откроется.\n\n"
            "📌 Ваш ID нужен администратору, чтобы добавить вас в список доступа.",
            parse_mode="HTML",
        )
        return
    await state.set_state(KPD.product_choice)
